...
export type ActiveTickerSource = "fdc3" | "manual";

/**
 * The "currently active" ticker for this OMS Order Entry instance - either the last symbol
 * broadcast by another MFE (e.g. a Watchlist row click) via FDC3's fdc3.instrument context, or a
 * manually-typed override, which also works standalone with no desktop agent connected at all.
 */
export function useActiveTicker() {
  const [ticker, setTicker] = useState<string | null>(null);
  const [source, setSource] = useState<ActiveTickerSource | null>(null);
